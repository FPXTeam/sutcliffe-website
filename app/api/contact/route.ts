...
export async function POST(request: Request) {
  try {
    ...
    const submitted = new Intl.DateTimeFormat("en-NZ", {
      dateStyle: "medium",
      timeStyle: "short",
    ...
    const text = [
      "New Sutcliffe Trading website enquiry",
      "",
    ...
    const response = await fetch(RESEND_ENDPOINT, {
      ...
      body: JSON.stringify({
        from: `Sutcliffe Trading Website <${from}>`,
        to: [to],
        reply_to: email,
        subject: `New website timber enquiry${company ? ` - ${company}` : ""}`,
        text,
      }),
    });
